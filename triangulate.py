...
def load_pose2d_fnames(fname_dict, offsets_dict=None, cam_names=None):
    ...
    for ix_cam, (cam_name, pose_name) in enumerate(zip(cam_names, pose_names)):
        dlabs = pd.read_excel(pose_name, header=None)
        bodyparts = (dlabs.loc[0])
        joint_names = list(bodyparts.dropna())[1:]
        dlabs = dlabs.loc[2:,1:]
        datas.append(dlabs)
    ...
    for cam_ix, dlabs in enumerate(datas):
        ...
        for joint_ix, joint_name in enumerate(joint_names):
            try:
                points[cam_ix, :, joint_ix] = np.array(dlabs.loc[:, i:i+1])
                scores[cam_ix, :, joint_ix] = np.array(dlabs.loc[:, i+2]).ravel()
                i += 3
            except KeyError:
                pass
    return {
...
# removed video_folder, pose_folder from def
    cam_names = sorted(fname_dict.keys())
...
def process_session(config, session_path):
    ...
    for name in vid_names:
        ...
        try:
            triangulate(config, calib_folder, fname_dict, output_fname)
        except ValueError:
